...
**Fix**: Restrict the origins to trusted domains instead of allowing all origins.

---

### [17]: [ISSUE](java:S4488)
**Issue**: Replace `@RequestMapping(method = RequestMethod.GET)` with `@GetMapping`.  
**Fix**: Use `@GetMapping` for better readability and conciseness.
...
**Fix**: Use `@PostMapping` for better readability and conciseness.
...
**Fix**: Use `@DeleteMapping` for better readability and conciseness.
