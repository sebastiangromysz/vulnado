...
1. **CORS Fixes**: Restricted origins to `https://trusted-domain.com` for better security.
2. **Mapping Annotations**: Replaced `@RequestMapping` with `@GetMapping`, `@PostMapping`, and `@DeleteMapping` for modern conventions.
3. **Encapsulation**: Made `username` and `body` private and added getter and setter methods to adhere to encapsulation principles.

These changes ensure the code is secure, adheres to modern conventions, and follows Java best practices.
