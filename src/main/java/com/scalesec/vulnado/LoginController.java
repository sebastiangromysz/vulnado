...
The `@CrossOrigin(origins = "*")` annotation enables CORS for all origins, which can be a security risk. To mitigate this, we should restrict the origins to trusted domains. For example, replace `"*"` with specific domains like `"http://example.com"`.
